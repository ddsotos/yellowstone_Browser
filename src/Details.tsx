    ...
    <main className="details-page">
      ...
      <section>
        ...
        <ol>
          ...
          <li>AI分析モードでは、自分の手とAI上位3択を押して盤面を切り替えます。</li>
      ...
      <section>
        <h2>AI推定勝率</h2>
        <p>
          表示値は厳密な勝率計算ではなく、heuristic同士の4人戦データから学習したモデルによる
          推定です。同率勝者は勝利を人数で分割して学習しています。通常NPC戦に合わせたモデルのため、
          強化NPC戦の値は未較正です。表示は整数に丸めますが、最善手の比較には丸め前の値を使います。
        </p>
        <p>
          2枚プレイでは、補充によるランダムなカードをまだ引かず、
          「山札から補充／補充しない／マイナスカードから補充」の判断を明示した状態で評価します。
          直近3手番、枠の移動、公開されているマイナスカード情報もモデル入力に含みます。
        </p>
        <p>
          AI上位3択は、プレイするカードの組み合わせまたは補充判断が異なる候補を表示します。
          同じカードで枠・プレイ順だけが異なる候補はまとめ、その中で最も推定勝率が
          高い手を代表として表示します。山札から補充、補充しない、マイナスカードから補充は
          それぞれ別候補として扱います。自分と同じカードの候補も除外せず表示します。
        </p>
        <p>
          現在のV2モデルは開発プレビューです。同一seedの1,000ゲーム評価では勝率26.03%で、
          比較用の旧+6hモデルの29.08%を下回っています。入力表現の改善確認を優先したモデルであり、
          表示値と候補順位は参考情報として扱ってください。
        </p>
        <p>
        ...
        <dl>
          <dt>モデル</dt>
          <dd>V2 generation 0 / 197,800 games / epoch 001</dd>
          <dt>入力正規化</dt>
          <dd>strict_residual_v2</dd>
          <dt>test Brier score</dt>
          <dd>0.1300</dd>
          <dt>test log loss</dt>
          <dd>0.4187</dd>
        </dl>
